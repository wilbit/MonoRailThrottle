...
using MonoRailThrottle.Attributes;
using MonoRailThrottle.Logging;
using MonoRailThrottle.Models;
using MonoRailThrottle.Net;
using MonoRailThrottle.Repositories;

namespace MonoRailThrottle
{
    //INFO по сути это не атрибут в понятии монорейл, это имплементация атрибута
    /// <summary>
    /// Throttle action filter
    /// </summary>
    public class ThrottlingFilter : ActionFilterAttribute, IActionFilter//FilterAttribute
    {
            ...
            if (Policy != null && applyThrottling)
            {
                ...
                if (!_core.IsWhitelisted(identity))
                {
                    ...
                    foreach (var rate in defRates)
                    {
                        ...
                        TimeSpan timeSpan = _core.GetTimeSpanFromPeriod(rateLimitPeriod);
                        ...
                        if (rateLimit > 0)
                        {
                            // increment counter
                            string requestId;
                            var throttleCounter = _core.ProcessRequest(identity, timeSpan, rateLimitPeriod, out requestId);
                            ...
                            if (throttleCounter.TotalRequests > rateLimit)
                            {
                                ...
                                if (Logger != null)
                                {
                                    Logger.Log(_core.ComputeLogEntry(requestId, identity, throttleCounter, rateLimitPeriod.ToString(), rateLimit, actionContext.Request));
                                }
        ...
        protected virtual RequestIdentity SetIdentity(HttpRequestMessage request)
        {
            ...
            entry.ClientIp = _core.GetClientIp(request).ToString();
